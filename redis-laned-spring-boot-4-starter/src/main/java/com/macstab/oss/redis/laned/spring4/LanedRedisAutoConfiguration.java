...
public class LanedRedisAutoConfiguration {
  ...
  public RedisConnectionFactory lanedRedisConnectionFactory(
      final DataRedisProperties redisProperties,
  ...
  private boolean isSentinelConfigured(final DataRedisProperties props) {
  ...
  private boolean isClusterConfigured(final DataRedisProperties props) {
    final var cluster = props.getCluster();
    return cluster != null && cluster.getNodes() != null && !cluster.getNodes().isEmpty();
  }

  private RedisStandaloneConfiguration buildStandaloneConfig(final DataRedisProperties props) {
  ...
  private RedisSentinelConfiguration buildSentinelConfig(final DataRedisProperties props) {
    final var sentinel = props.getSentinel();
    final var config =
  ...
  private RedisClusterConfiguration buildClusterConfig(final DataRedisProperties props) {
    final var cluster = props.getCluster();
   ...
   *   <li><strong>Timeouts:</strong> {@link DataRedisProperties#getTimeout()} and {@link
   *       DataRedisProperties#getConnectTimeout()}
  ...
  private LettuceClientConfiguration buildClientConfiguration(
      final DataRedisProperties properties,
      final ObjectProvider<SslBundles> sslBundles,
      final ObjectProvider<ClientResources> clientResources,
      final ObjectProvider<LettuceClientConfigurationBuilderCustomizer> customizers) {
  ...
  private Optional<ClientOptions> buildClientOptions(
      final DataRedisProperties properties, final ObjectProvider<SslBundles> sslBundles) {
